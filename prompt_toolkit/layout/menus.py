...
from rp.prompt_toolkit.filters import HasCompletions, IsDone, Condition, to_cli_filter
...
def _trim_text(text, max_width):
    ...
    if width > max_width:
        ...
        if len(text) == width:
            trimmed_text = (text[:max(1, max_width-4)] + '...')[:max_width]
        ...
        # inefficient, but ok for now.)
            ...
            trimmed_text = ''
            for c in text:
                if get_cwidth(trimmed_text + c) <= max_width - 4:
                    trimmed_text += c
            trimmed_text += '...'
